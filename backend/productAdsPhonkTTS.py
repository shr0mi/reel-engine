...
# Local output directory (matches backend/routers/product_ads_router.py).
# Audio is always written to <PHONK_DIR>/audio.mp3 and overwrites any
# previous file — single canonical artifact for the latest generation.
PHONK_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "temp-phonks",
)
OUTPUT_FILENAME = "audio.mp3"
OUTPUT_PATH = os.path.join(PHONK_DIR, OUTPUT_FILENAME)
...
async def process_tts_and_upload(script_text: str, language: str) -> dict:
    ...
    and writes the audio to backend/temp-phonks/audio.mp3 (overwriting).
    """
    ...
    # 2. Initialize edge-tts speech pipeline (+15% rate works great for pacing phonk drops)
    ...
    # 4. Ensure the target directory exists, then overwrite audio.mp3 in place
    os.makedirs(PHONK_DIR, exist_ok=True)
    with open(OUTPUT_PATH, "wb") as out_file:
        out_file.write(bytes(audio_bytes))

    # 5. Step through word lists in strides of 3 to create the custom caption payload
    captions_list = []
    caption_id = 1
